...
используется собственный eslint plugin *eslint-plugin-dm-fsd-rules*,
...
Разрешено использование feature flags только с помощью хулпера toggleFeatures
...
1. Название фича-флага для удаления
2. Состояние (on/off)

Пример, для удаления фичи isCounterEnabled с состоянием "on" нужно запустить 
скрипт npx ts-node ./scripts/remove-feature.ts isCounterEnabled on
